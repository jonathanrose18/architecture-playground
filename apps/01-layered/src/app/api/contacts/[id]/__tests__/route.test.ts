...
describe('GET /api/contacts/[id]', () => {
   ...
   it('responds with 200 and the contact when found', async () => {
      ...
      const response = await GET(
         new Request('http://localhost/api/contacts/contact-1') as NextRequest,
   ...
   it('responds with 500 when the service throws', async () => {
      ...
      const response = await GET(
         new Request('http://localhost/api/contacts/missing') as NextRequest,
...
describe('PUT /api/contacts/[id]', () => {
   ...
   it('responds with 200 and the updated contact for a valid payload', async () => {
      ...
      const response = await PUT(
         makePutRequest({ firstName: 'Jane', email: 'jane@example.com' }),
         makeContext() as never
      );
      const data = await response.json();
