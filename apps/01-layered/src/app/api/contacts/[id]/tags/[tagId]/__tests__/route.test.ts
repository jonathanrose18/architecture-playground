...
describe('DELETE /api/contacts/[id]/tags/[tagId]', () => {
	...
	it('responds with 204 and removes the tag assignment', async () => {
		...
		const response = await DELETE(new Request('http://localhost/api/contacts/contact-1/tags/tag-1') as NextRequest, makeContext() as never);
	...
	it('responds with 500 when the service throws', async () => {
		...
		const response = await DELETE(new Request('http://localhost/api/contacts/contact-1/tags/missing') as NextRequest, makeContext('contact-1', 'missing') as never);
